...
- `GET /api/admin/mega-draw` returns `{ status: "SUCCESS", configuration, lifecycle? }` for the execution year's current Mega Draw lifecycle. `lifecycle` contains status, locked snapshots when selection has started, selected rows in chronological selection order, next reverse prize ordinal, remaining ordered prizes, and completion data when complete or closed. It never returns Mega audit, reset, void, redraw, replacement, or history data.
